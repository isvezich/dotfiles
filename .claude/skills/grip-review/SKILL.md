---
name: grip-review
description: Serve a Markdown file (plan, spec, design doc) via ~/bin/grip on a non-localhost URL so the user can review it from any device on their LAN. Use whenever a superpowers skill (brainstorming spec review, writing-plans plan review, executing-plans checkpoint, etc.) asks the user to review a `.md` file. Returns a URL; print it on its own line before asking for review.
when_to_use: Any time you are about to ask the user to review a markdown plan or spec file produced by a superpowers skill — the moment before you ask "please review", run this skill against the file path, then include the URL in your prompt to the user.
version: 1.0.0
languages: bash
---
...
Serves a markdown file via the local `~/bin/grip` wrapper, which binds `go-grip` to the user's LAN-visible IP. The URL it returns is reachable from any device on their network, so reviews don't require sitting at this machine.
...
At every superpowers review gate that asks the user to read a `.md` file:
...
On failure the script exits non-zero and prints a diagnostic to stderr. If that happens, fall back to asking the user to read the file directly.
